...
type GenerationVersion = { id: string; title: string; label: string };
type GenerationRow = { id: string; order_id: string; versionId: string | null; versions: GenerationVersion[]; provider: string; model: string; status: string; error_code: string | null; reserved_credits_millis: number; created_at: string; completed_at: string | null; recipientName: string; ownerEmail: string; orderStatus: string; shareable: boolean; showcased: boolean };
...
type UserForm = { email: string; password: string; displayName: string; whatsapp: string; role: "user" | "support" | "admin"; status: "active" | "suspended"; reason: string };
const emptyUser: UserForm = { email: "", password: "", displayName: "", whatsapp: "", role: "user", status: "active", reason: "" };
...
export function AdminDashboard({ adminEmail }: { adminEmail: string }) {
  ...
  const [showcaseVersionId, setShowcaseVersionId] = useState("");
  ...
  const filteredUsers = useMemo(() => {
    ...
    return (data?.users ?? []).filter((user) => `${user.displayName} ${user.email} ${user.whatsapp} ${user.role}`.toLocaleLowerCase("pt-BR").includes(query));
  }, [data?.users, search]);

  const openCreate = () => { setUserForm(emptyUser); setSelectedUser(null); setUserDialog("create"); setMessage(""); setError(""); };
  const openEdit = (user: UserRow) => {
    setSelectedUser(user);
    setUserForm({ email: user.email, password: "", displayName: user.displayName || "Cliente Te Cantei", whatsapp: user.whatsapp || "", role: user.role, status: user.status === "suspended" ? "suspended" : "active", reason: "" });
    setUserDialog("edit"); setMessage(""); setError("");
  };

  const saveUser = async (event: FormEvent) => {
    event.preventDefault(); setBusy(true); setError("");
    const whatsapp = normalizeAdminWhatsapp(userForm.whatsapp);
    if (userForm.whatsapp.trim() && !whatsapp) {
      setBusy(false); setError("Informe um WhatsApp válido com DDD."); return;
    }
    ...
    const response = await fetch(editing ? `/api/admin/users/${selectedUser.id}` : "/api/admin/users", {
      ...
      body: JSON.stringify({ ...userForm, whatsapp: whatsapp ?? "" }),
    }).catch(() => null);
    setBusy(false);
    if (!response?.ok) {
  ...
  const openShareDialog = (generation: GenerationRow) => {
    ...
    setShareVersionId(generation.versions[0]?.id ?? "");
    setShareReason("");
    setShareDedication("");
    setShareError("");
    setShareUrl("");
  ...
  const generateShareLink = async () => {
    if (!shareGeneration || !shareVersionId) return;
    setShareBusy(true);
    setShareError("");
    const response = await fetch(`/api/admin/orders/${shareGeneration.order_id}/share`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ versionId: shareVersionId, dedication: shareDedication, reason: shareReason }),
    }).catch(() => null);
    ...
    if (!response?.ok || !payload?.shareUrl) {
      setShareError(
        payload?.error === "version_not_ready"
          ? "Esta versão ainda não possui a música completa armazenada."
  ...
  const openShowcaseDialog = (row: GenerationRow, mode: "add" | "remove") => {
    ...
    setShowcaseVersionId(row.versions[0]?.id ?? "");
    setShowcaseReason("");
    setShowcaseError("");
  };
  ...
  const closeShowcaseDialog = () => {
    ...
    setShowcaseVersionId("");
    setShowcaseReason("");
    setShowcaseError("");
  };

  const confirmShowcaseAction = async () => {
    if (!showcaseAction || (showcaseAction.mode === "add" && !showcaseVersionId)) return;
    ...
    const response = await fetch(mode === "add" ? "/api/admin/showcase" : `/api/admin/showcase/${row.order_id}`, {
      ...
      body: JSON.stringify(mode === "add"
        ? { orderId: row.order_id, versionId: showcaseVersionId, reason: showcaseReason }
    ...
    <main className="min-h-screen bg-[#f6f3f4] text-[#2a1720]">
            ...
            <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as AdminTab)} className="gap-6">
              ...
              <TabsContent value="overview" className="space-y-6">
                ...
                <div className="grid gap-5 xl:grid-cols-[1.35fr_1fr]">
                  <Card className="rounded-[24px] border-black/5 shadow-none"><CardHeader><CardTitle className="font-display text-xl">Acessos · {data.period.label}</CardTitle><CardDescription>Visitantes únicos por sessão e visualizações de páginas.</CardDescription></CardHeader><CardContent><ChartContainer className="h-[280px] w-full aspect-auto" config={{ visits: { label: "Visualizações", color: "#F0BD4F" }, visitors: { label: "Visitantes", color: "#FFE49A" } }}><AreaChart data={data.series}><defs><linearGradient id="visitsFill" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#F0BD4F" stopOpacity={0.35}/><stop offset="95%" stopColor="#F0BD4F" stopOpacity={0.02}/></linearGradient></defs><CartesianGrid vertical={false} /><XAxis dataKey="date" tickFormatter={shortDate} minTickGap={28}/><YAxis allowDecimals={false} width={28}/><ChartTooltip content={<ChartTooltipContent />} /><Area dataKey="visits" type="monotone" stroke="#F0BD4F" fill="url(#visitsFill)" strokeWidth={2}/><Area dataKey="visitors" type="monotone" stroke="#FFE49A" fill="transparent" strokeWidth={2}/></AreaChart></ChartContainer></CardContent></Card>
                  <Card className="rounded-[24px] border-black/5 shadow-none"><CardHeader><CardTitle className="font-display text-xl">Vendas · {data.period.label}</CardTitle><CardDescription>Receita confirmada com base na atualização do pagamento.</CardDescription></CardHeader><CardContent><ChartContainer className="h-[280px] w-full aspect-auto" config={{ revenueCents: { label: "Receita", color: "#F0BD4F" } }}><BarChart data={data.series}><CartesianGrid vertical={false}/><XAxis dataKey="date" tickFormatter={shortDate} minTickGap={28}/><YAxis tickFormatter={(value) => `R$${Number(value)/100}`} width={52}/><ChartTooltip content={<ChartTooltipContent formatter={(value) => money(Number(value))} />} /><Bar dataKey="revenueCents" fill="#F0BD4F" radius={[6,6,0,0]}/></BarChart></ChartContainer></CardContent></Card>
                </div>
        ...
        <DialogContent className="rounded-[24px] sm:max-w-xl">
          ...
          <div className="mt-5 space-y-4">
            <p className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-950">
              Este é um link administrativo para a versão escolhida. Ele funciona mesmo antes do pagamento, fica registrado na auditoria e <strong>não libera a entrega na conta do cliente</strong>. Um novo link invalida somente o link administrativo anterior deste pedido.
            </p>

            {!shareUrl && (
              <>
                <Field label="Música do link">
                  <NativeSelect className="w-full" value={shareVersionId} onChange={(event) => setShareVersionId(event.target.value)}>
                    {shareGeneration?.versions.map((version) => <NativeSelectOption key={version.id} value={version.id}>{version.label} · {version.title}</NativeSelectOption>)}
                  </NativeSelect>
                </Field>
                <Field label="Dedicatória (opcional)">
                  <Textarea maxLength={500} value={shareDedication} onChange={(event) => setShareDedication(event.target.value)} placeholder="Se vazio, usa uma mensagem padrão" />
                </Field>
                ...
                <Button type="button" className="w-full rounded-full bg-[#7e2148]" disabled={shareBusy || !shareVersionId || shareReason.trim().length < 8} onClick={() => void generateShareLink()}>
      ...
      <Dialog open={Boolean(showcaseAction)} onOpenChange={(open) => !open && closeShowcaseDialog()}>
        <DialogContent className="rounded-[24px] sm:max-w-xl">
          ...
          <div className="mt-5 space-y-4">
            {showcaseAction?.mode === "add" && (
              <Field label="Música para destacar">
                <NativeSelect className="w-full" value={showcaseVersionId} onChange={(event) => setShowcaseVersionId(event.target.value)}>
                  {showcaseAction.row.versions.map((version) => <NativeSelectOption key={version.id} value={version.id}>{version.label} · {version.title}</NativeSelectOption>)}
                </NativeSelect>
              </Field>
            )}
            ...
            <Button type="button" className="w-full rounded-full bg-[#7e2148]" disabled={showcaseBusy || (showcaseAction?.mode === "add" && !showcaseVersionId) || showcaseReason.trim().length < 8} onClick={() => void confirmShowcaseAction()}>
...
function normalizeAdminWhatsapp(value: string) { let digits = value.replace(/\D/g, ""); if (!digits) return null; if (digits.length === 10 || digits.length === 11) digits = `55${digits}`; return /^55[1-9][0-9]{9,10}$/.test(digits) ? `+${digits}` : null; }
