...
import { deleteKieWebhookHmacKey, putKieWebhookHmacKey } from "@/lib/admin/secrets";
...
const saveSchema = z.object({
  ...
  hmacKey: z.string().trim().min(32).max(512),
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await putKieWebhookHmacKey(createSupabaseAdminClient(), input.data.hmacKey, identity.id, input.data.reason);
...
export async function DELETE(request: Request) {
  ...
  try {
    const identity = await getAdminIdentity();
    if (!identity) return NextResponse.json({ error: "not_found" }, { status: 404, headers: NO_STORE });
    await deleteKieWebhookHmacKey(createSupabaseAdminClient(), identity.id, input.data.reason);
