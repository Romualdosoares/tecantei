...
## Continuação de 14/09/2026 — webhook Pix Efí direto na Vercel

Autorização do proprietário: substituir o webhook de produção anteriormente vinculado a `payments.ggcheckout.app` pelo endpoint do Te Cantei. A rota Efí passou a admitir o modo direto oficialmente documentado para hospedagem compartilhada (`skip-mTLS`), exigindo simultaneamente o HMAC secreto e o IP de callbacks publicado pela Efí; a Vercel sobrescreve `x-forwarded-for` com o IP real da conexão. O callback continua sem autoridade isolada para liberar a música: cada notificação provoca uma nova consulta mTLS autenticada à API Pix e somente o estado, valor, moeda e identificador confirmados são aplicados de forma idempotente. O modo anterior por gateway mTLS foi preservado como alternativa. A primeira tentativa de cadastro revelou que a sondagem da Efí não envia uma lista `pix`; a rota passou a reconhecer esse teste autenticado sem executar transição financeira. Testes específicos de IP verdadeiro/falso e da sondagem, lint e build de produção foram aprovados. A revisão foi publicada e a Efí aceitou a sondagem; uma consulta posterior confirmou o webhook de produção em `tecantei.vercel.app/api/payments/webhooks/efi`, com HMAC e parâmetro `ignorar`, substituindo o endereço da GGCheckout.

## Continuação de 14/09/2026 — estúdio digital e CTA na amostra

A experiência de produção e audição da prévia foi modernizada em preto e dourado. O antigo disco de vinil das telas de andamento e conclusão foi substituído por uma interface de estúdio digital com espectro animado, ondas de sinal, indicadores técnicos e linhas metálicas nítidas, sem blur. Os cards dos players privados também receberam acabamento coerente com a identidade premium.
...
Validação: ESLint, TypeScript, `git diff --check` e build local de produção Next.js 16.3.5 foram aprovados, com 24 páginas geradas. Após o token novo ter sido cadastrado pelo responsável na Vercel, o deploy `dpl_FjukNGRkVQDUB3SMSZiQKg7y2JHA` foi promovido para `www.tecantei.site`. Uma visita controlada à Home retornou HTTP 200 e acionou o fluxo de `PageView`; Pixel e fallback continuam presentes. A confirmação da recepção e deduplicação pela Meta depende da atualização no Gerenciador de Eventos, pois a API não expõe a credencial nem a resposta ao navegador.

## Auditoria do evento Purchase antes de anunciar em 16/09/2026

Pedido: verificar valor, moeda, momento de disparo, deduplicação e confirmação assíncrona do evento `Purchase` antes de iniciar anúncios.

Resultado: **não está pronto para anunciar com otimização em Purchase**. A configuração pública do site responde `productPriceCents: 1990` e `currency: "BRL"`; o checkout também valida valor e moeda recebidos do provedor antes de persistir o pagamento. Os webhooks Pix de Mercado Pago e Efí consultam a cobrança autenticada e aplicam o estado de forma idempotente, portanto a confirmação assíncrona do Pix é tratada no domínio de pagamentos.
